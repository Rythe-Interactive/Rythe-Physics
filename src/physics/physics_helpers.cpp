...
namespace rythe::physics
{
	...
	void PhysicsHelpers::createPhysicsMaterial(
		float dynamicFriction, float staticFriction, float restitution, const char* name
	)
	{
		rsl::size_type hashResult = rsl::nameHash(name);

		m_physicsMaterialHashes.push_back(hashResult);
		events::EventBus::raiseEvent<request_create_physics_material>(
			request_create_physics_material{dynamicFriction, staticFriction, restitution, hashResult}
		);
	}
	...
	rsl::size_type PhysicsHelpers::retrievePhysicsMaterialHash(const char* name)
	{
		...
		log::warn(
			"Attempted to retrieve non existing physics material with name {0}, default material was returned instead ",
			name
		);
