...
namespace rythe::physics
{
	...
	public:
		 ...
		 */
		static void GetSupportPoint(const rsl::math::float3& planePosition, const rsl::math::float3& direction, ConvexCollider* collider, const math::float4x4& colliderTransform, rsl::math::float3& worldSupportPoint)
		{
		 ...
		 * @return returns true if a seperating axis was found
		 */
		static void GetSupportPointNoTransform(rsl::math::float3 planePosition, rsl::math::float3 direction, ConvexCollider* collider, const math::float4x4& colliderTransform, rsl::math::float3& worldSupportPoint);



		/** @brief Given a std::vector of vertices, gets the support point in the given direction
		 */
		static float GetSupportPoint(const std::vector<rsl::math::float3>& vertices, const rsl::math::float3& direction, rsl::math::float3& outVec);
		 ...
		 * @return returns true if a seperating axis was found
		 */
		static bool FindSeperatingAxisByExtremePointProjection(ConvexCollider* convexA, ConvexCollider* convexB, const math::float4x4& transformA, const math::float4x4& transformB, PointerEncapsulator<HalfEdgeFace>& refFace, float& maximumSeperation, bool shouldDebug = false);
		 ...
		 */
		static bool DetectConvexSphereCollision(ConvexCollider* convexA, const math::float4x4& transformA, rsl::math::float3 sphereWorldPosition, float sphereRadius, float& maximumSeperation);
		 ...
		 */
		static std::pair<rsl::math::float3, rsl::math::float3> CombineAABB(const std::pair<rsl::math::float3, rsl::math::float3>& first, const std::pair<rsl::math::float3, rsl::math::float3>& second);
		 ...
		 *
		 */
		static void SutherlandHodgmanFaceClip(rsl::math::float3& planeNormal, rsl::math::float3& planePosition, std::vector<ContactVertex>& inputList, std::vector<ContactVertex>& outputList, HalfEdgeEdge* clippingEdge)
		{
			for (size_t i = 0; i < inputList.size(); i++)
			{
				...
				if (math::abs(currentVertDistFromPlane) < constants::sutherlandHodgmanClippingThreshold || math::abs(nextVertDistFromPlane) < constants::sutherlandHodgmanClippingThreshold)
				{
				...
				const rsl::math::float3& pointAbovePlane = isCurrentVertexUnderPlane ? nextVertex.position : currentVertex.position;
				const rsl::math::float3& pointBelowPlane = isCurrentVertexUnderPlane ? currentVertex.position : nextVertex.position;
				...
				else if (!isCurrentVertexUnderPlane && isNextVertexUnderPlane)
				{
					...
					if (FindLineToPlaneIntersectionPoint(planeNormal, planePosition, pointBelowPlane, pointAbovePlane, intersectionPoint))
					{
		 ...
		 */
		static void FindClosestPointsBetweenLineSegments(rsl::math::float3 p1, rsl::math::float3 p2, rsl::math::float3 p3, rsl::math::float3 p4, rsl::math::float3& outp1p2, rsl::math::float3& outp3p4)
		{
		 ...
		 */
		static float PointDistanceToPlane(const rsl::math::float3& planeNormal, const rsl::math::float3& planePosition, const rsl::math::float3& point)
		{
		 ...
		 */
		static bool IsPointAbovePlane(const rsl::math::float3& planeNormal, const rsl::math::float3& planePosition, const rsl::math::float3& point)
		{
		 ...
		 */
		static bool FindLineToPlaneIntersectionPoint(const rsl::math::float3& planeNormal, const rsl::math::float3& planePosition, const rsl::math::float3& startPoint, const rsl::math::float3& endPoint, rsl::math::float3& intersectionPoint)
		{
		 ...
		 * @return Returns true if an intersection is found
		 */
		static bool FindLineToPlaneIntersectionPoint(const rsl::math::float3& planeNormal, const rsl::math::float3& planePosition, const rsl::math::float3& startPoint, const rsl::math::float3& endPoint, rsl::math::float3& intersectionPoint, float& interpolant)
		{
			rsl::math::float3 direction = endPoint - startPoint;
		 ...
		 */
		static float FindLineToPlaneInterpolant(const rsl::math::float3& startPoint, const rsl::math::float3& endPoint, const rsl::math::float3& planePosition, const rsl::math::float3& planeNormal)
		{
		 ...
		 */
		static float FindClosestPointToLineInterpolant(const rsl::math::float3& startPoint, const rsl::math::float3& lineDirection, const rsl::math::float3& pointPosition);

		static rsl::math::float3 FindClosestPointToLineSegment(const rsl::math::float3& start, const rsl::math::float3& end, const rsl::math::float3& pointPosition);
		 ...
		 */
		static std::vector<std::vector<math::float4>> GenerateVoronoi(std::vector<rsl::math::float3> points, math::float2 xRange, math::float2 yRange, math::float2 zRange, rsl::math::float3 containerResolution, bool xPeriodic = false, bool yPeriodic = false, bool zPeriodic = false, int initMem = 8)
		{
		 ...
		 */
		static bool CollideAABB(const rsl::math::float3 low0, const rsl::math::float3 high0, const rsl::math::float3 low1, const rsl::math::float3 high1)
		{
			return low0.x <= high1.x && high0.x >= low1.x &&
				   low0.y <= high1.y && high0.y >= low1.y && low0.z <= high1.z && high0.z >= low1.z;
		}
		 ...
		 * @return Whether there is collision
		 */
		static bool CollideAABB(const std::pair<rsl::math::float3, rsl::math::float3> col0, const std::pair<rsl::math::float3, rsl::math::float3> col1)
		{
		 ...
		 */
		static void calculateNewellPlane(const std::vector<rsl::math::float3>& v, rsl::math::float3& outPlaneNormal, float& distToCentroid);
		 ...
		 */
		static bool isNewellFacesCoplanar(HalfEdgeFace* first, HalfEdgeFace* second, HalfEdgeEdge* connectingEdge, float scalingEpsilon, rsl::math::float3& outNormal, int skipCount);
	...
	private:
		 ...
		 */
		static bool attemptBuildMinkowskiFace(HalfEdgeEdge* edgeA, HalfEdgeEdge* edgeB, const math::float4x4& transformA, const math::float4x4& transformB);
		 ...
		 */
		static bool isMinkowskiFace(const rsl::math::float3& transformedA1, const rsl::math::float3& transformedA2, const rsl::math::float3& transformedB1, const rsl::math::float3& transformedB2, const rsl::math::float3& planeANormal, const rsl::math::float3& planeBNormal);
		 ...
		 */
		static bool buildInitialHull(const std::vector<rsl::math::float3>& vertices, std::array<rsl::math::float3, 6>& supportPoints, std::vector<HalfEdgeFace*>& outFaces);
		 ...
		 */
		static void createHalfEdgeFaceFromEyePoint(const rsl::math::float3 eyePoint, const std::vector<HalfEdgeEdge*>& reversedEdges, std::vector<HalfEdgeFace*>& createdFaces);
		 ...
		 */
		static void partitionVerticesToList(const std::vector<rsl::math::float3> vertices, std::list<ColliderFaceToVert>& outFacesWithOutsideVerts);
		 ...
		 */
		static void findHorizonEdgesFromFaces(const rsl::math::float3& eyePoint, std::vector<HalfEdgeFace*>& faces, std::vector<HalfEdgeEdge*>& outHorizonEdges, float scalingEpsilon);

		/** @brief Given an eyePoint, and a convex hull represented by 'facesWithOutsideVerts', merged the eyePoint
		 * into the hull.
		 */
		static void mergeVertexToHull(const rsl::math::float3& eyePoint, std::list<ColliderFaceToVert>& facesWithOutsideVerts, float scalingEpsilon);
