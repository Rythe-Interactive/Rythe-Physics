...
namespace rythe::physics
{
    ...
    std::shared_ptr<ConvexCollider> PhysicsStatics::generateConvexHull(const std::vector<rsl::math::float3>& vertices)
    {
        ...
        GetSupportPoint(vertices, rsl::math::float3(1, 0, 0), supportVertices.at(0));
        GetSupportPoint(vertices, rsl::math::float3(-1, 0, 0), supportVertices.at(1));

        //Get support points in -y and y
        GetSupportPoint(vertices, rsl::math::float3(0, 1, 0), supportVertices.at(2));
        GetSupportPoint(vertices, rsl::math::float3(0, -1, 0), supportVertices.at(3));

        //Get support points in -z and z
        GetSupportPoint(vertices, rsl::math::float3(0, 0, 1), supportVertices.at(4));
        GetSupportPoint(vertices, rsl::math::float3(0, 0, -1), supportVertices.at(5));
    ...
    bool PhysicsStatics::isMinkowskiFace(const rsl::math::float3& transformedA1, const rsl::math::float3& transformedA2, const rsl::math::float3& transformedB1,
        const rsl::math::float3& transformedB2, const rsl::math::float3& planeANormal, const rsl::math::float3& planeBNormal)
    {
    ...
    bool PhysicsStatics::buildInitialHull(const std::vector<rsl::math::float3>& vertices,
        std::array<rsl::math::float3,6>& supportVertices, std::vector<HalfEdgeFace*>& faces )
    {
        //Summary:
        ...
        rsl::math::float3& firstDistant = supportVertices.at(0);
        rsl::math::float3& secondDistant = supportVertices.at(1);

        //Iterate through support vertices to find the combination of vertices that represent the 2 most distant points
        for (rsl::size_type i = 0; i < supportVertices.size(); i++)
        {
            rsl::math::float3& first = supportVertices.at(i);

            for (rsl::size_type j = i+1; j < supportVertices.size()-1; j++)
            {
                rsl::math::float3& second = supportVertices.at(j);
        ...
        rsl::math::float3 firstToSecond = math::normalize(secondDistant - firstDistant);

        const rsl::math::float3* thirdDistant = nullptr;
        ...
        mostDistant = std::numeric_limits<float>::lowest();
        const rsl::math::float3* firstEyePoint = nullptr;

        rsl::math::float3 planePosition = initialFace->centroid;
        rsl::math::float3 planeNormal = initialFace->normal;
        ...
        rsl::math::float3 eyePoint = *firstEyePoint;
    ...
    void PhysicsStatics::createHalfEdgeFaceFromEyePoint(const rsl::math::float3 eyePoint, const std::vector<HalfEdgeEdge*>& reversedEdges, std::vector<HalfEdgeFace*>& createdFaces)
    {
        ...
        for (auto edge : reversedEdges)
        {
            ...
            //initialize new face
            rsl::math::float3 faceNormal = math::normalize(math::cross(nextPairing->edgePosition - pairing->edgePosition,
    ...
    void PhysicsStatics::partitionVerticesToList(const std::vector<rsl::math::float3> vertices,
    ...
    void PhysicsStatics::findHorizonEdgesFromFaces(const rsl::math::float3& eyePoint, std::vector<HalfEdgeFace*>& faces,
    ...
    void PhysicsStatics::mergeVertexToHull(const rsl::math::float3& eyePoint,std::list<ColliderFaceToVert>& facesWithOutsideVerts,
        float scalingEpsilon)
    {
        ...
        std::vector<rsl::math::float3> unmergedVertices;
        ...
        for (auto listIter = facesWithOutsideVerts.begin(); listIter != facesWithOutsideVerts.end();)
        {
            ...
            const rsl::math::float3& planeCentroid = face->centroid;
            const rsl::math::float3& planeNormal = face->normal;
    ...
    bool PhysicsStatics::isNewellFacesCoplanar(HalfEdgeFace* first, HalfEdgeFace* second,
        HalfEdgeEdge* connectingEdge, float scalingEpsilon,  rsl::math::float3& outNormal,int skipCount)
    {
