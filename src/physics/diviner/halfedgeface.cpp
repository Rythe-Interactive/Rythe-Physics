...
{
	HalfEdgeFace::HalfEdgeFace(HalfEdgeEdge* newStartEdge, rsl::math::float3 newNormal)
	...
	void HalfEdgeFace::initializeFace()
	{
		...
		auto initializeEdgeToFaceFunc = [this, &currentEdgeId, edgeCount](HalfEdgeEdge* edge)
		{
			...
			EdgeLabel label(std::make_pair(faceCount, currentEdgeId), std::make_pair(faceCount, nextID));
