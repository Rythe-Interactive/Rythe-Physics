...
namespace rythe::physics::diviner
{
	struct rigidbody
	{
		...
		void serialize(Archive& archive)
		{
			...
			archive(
				cereal::make_nvp("Name", std::string("diviner::rigidbody")),
				cereal::make_nvp("Inverse Mass", inverseMass), cereal::make_nvp("Velocity", velocity),
				cereal::make_nvp("Acceleration", acc), cereal::make_nvp("Linear Drag", linearDrag),
				cereal::make_nvp("Inverse Intertia Tensor", localInverseInertiaTensor),
				cereal::make_nvp("Angular Acceleration", angularAcc),
				cereal::make_nvp("Angular Velocity", angularVelocity), cereal::make_nvp("Angular Drag", angularDrag),
				cereal::make_nvp("Global Centre of Mass", globalCentreOfMass),
				cereal::make_nvp("Force Accumulator", forceAccumulator),
				cereal::make_nvp("Torque Accumulator", torqueAccumulator), cereal::make_nvp("Restitution", restitution),
		...
		static float calculateFriction(float frictionA, float frictionB) { return math::max(frictionA, frictionB); }

		/** @brief Pushes the diviner::rigidbody in the direction parallel to 'force' and equal to the
		 * length of 'force'.
		 * @note This does NOT cause torque
		 */
		void addForce(rsl::math::float3 force) { forceAccumulator += force; }
		...
		void setMass(float mass) { inverseMass = 1.0f / mass; }

		void setRestitution(float newRestitution) { restitution = math::clamp(newRestitution, 0.0f, 1.0f); }

		void setFriction(float newFriction) { friction = math::clamp(newFriction, 0.0f, 1.0f); }
